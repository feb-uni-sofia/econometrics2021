...
H_1: \beta \neq 0
$$
Implies that the difference between the expected group average scores are equal:
...
$$
T = \frac{\hat{\beta} - \beta_0}{\hat{SE}(\beta)}
$$
...
sim_h0 <- expand_grid(
  ...
  mutate(
    ...
    ## rnorm adds a value selected at random from a normal distribution with mean = 0 and standard
    ## deviation (sigma) = 19.85 (that we estimated from the sample)
    e = rnorm(number_of_samples * nrow(kids), mean = 0, sd = 19.85),
    kid_score = 77.54839 + 2 * mom_hs + e
  ) %>%
  group_by(R) %>%
  do(tidy(lm(kid_score ~ mom_hs, data = .))) %>%
