...
The expected value ($\mu$) is the location parameter of the distribution. It determines the location of the centre of the distribuion. The variance of the distribution determines the shape of the curve. Small values of $\sigma$ result in curves that are concentrated around the expected value ($\mu$). Large values fo $\sigma$ result in curves that are less concentrated around the expected value.
...
$$
\begin{align}
  y_i = 77.55 + 11.77 x_i + e_i   \label{eq:pop-reg}
\end{align}
$$

We would like to study the way the estimates for $\alpha$ and $\beta$ vary from sample to sample. For that purpose we will generate $R = 2000$ samples from the population described by \ref{eq:pop-reg}. The we will estimate the regression model for each of these 2000 samples. What follows is a lot of rather technical details that
...
We begin by introducing the `group_by` function and we use it to compute the sample group means for the two groups defined by `mom_hs`.
...
sim_df <- sim_df %>%
  mutate(
    ...
    kid_score = 77.54839 + 11.77 * mom_hs + rnorm(2000 * 434, mean = 0, sd = 20.41069)
  )
```
